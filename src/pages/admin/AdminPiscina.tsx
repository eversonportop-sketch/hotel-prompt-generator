...
const AdminPiscina = () => {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      // Sincroniza pool_hours em hotel_settings para manter compatibilidade
      const poolHoursValue = `${configForm.open_time} às ${configForm.close_time}`;
      await (supabase as any)
        .from("hotel_settings")
        .upsert({ key: "pool_hours", value: poolHoursValue }, { onConflict: "key" });

      toast.success("Configurações salvas!");
      qc.invalidateQueries({ queryKey: ["pool-config"] });
      qc.invalidateQueries({ queryKey: ["hotel_settings"] });
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-charcoal">
      ...
      <div className="p-6 md:p-10">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-body ${
              activeStatus === "open"
          ...
          {[
            {
              ...
              value: configForm?.max_capacity ?? config?.max_capacity ?? "—",
              color: "text-cream",
            },
            ...
            { label: "Horário fechamento", value: configForm?.close_time ?? "—", color: "text-primary" },
            {
              ...
              color: pendingIncidents > 0 ? "text-yellow-400" : "text-green-400",
            },
          ].map((s) => (
                ...
                <div className="max-w-2xl space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          value={configForm.open_time}
                          onChange={(e) => setConfigForm({ ...configForm, open_time: e.target.value })}
                        />
                      </div>
                      <div>
                        <label className="block text-xs uppercase tracking-widest text-primary/70 mb-1.5">
                        ...
                        <input
                          type="time"
                          className="w-full bg-black/50 border border-gold/20 rounded-lg px-4 py-3 text-cream text-sm focus:border-primary focus:outline-none transition"
                          value={configForm.close_time}
                          onChange={(e) => setConfigForm({ ...configForm, close_time: e.target.value })}
                        />
                      </div>
                      <div>
                        <label className="block text-xs uppercase tracking-widest text-primary/70 mb-1.5">
                        ...
                        <input
                          ...
                          min={1}
                          className="w-full bg-black/50 border border-gold/20 rounded-lg px-4 py-3 text-cream text-sm focus:border-primary focus:outline-none transition"
                          value={configForm.max_capacity}
                          onChange={(e) => setConfigForm({ ...configForm, max_capacity: Number(e.target.value) })}
                        />
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-primary/70 mb-2">
                        Status atual
                      </label>
                        ...
                        {Object.entries(POOL_STATUS).map(([key, val]) => {
                          ...
                          return (
                            <button
                              key={key}
                              type="button"
                              onClick={() => setConfigForm({ ...configForm, status: key })}
                  ...
                  <div className="bg-charcoal-light border border-gold/10 rounded-xl p-6 space-y-4">
                    ...
                    <div className="space-y-2">
                      {configForm.rules.map((rule, i) => (
                        <div key={i} className="flex items-center gap-2 group">
                          <input
                            className="flex-1 bg-black/50 border border-gold/10 rounded-lg px-3 py-2 text-cream text-sm focus:border-primary/40 focus:outline-none transition"
                            value={rule}
                            onChange={(e) => {
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setConfigForm({ ...configForm, rules: configForm.rules.filter((_, j) => j !== i) })
                            }
                    ...
                    <div className="flex gap-2">
                      <input
                        className="flex-1 bg-black/50 border border-gold/20 rounded-lg px-3 py-2 text-cream text-sm focus:border-primary/40 focus:outline-none transition"
                        value={newRule}
            ...
            {loadingIncidents ? (
              ...
              <div className="space-y-3">
                {incidents.map((inc, i) => (
                  ...
                  >
                    <div
                      className={`mt-1 w-2.5 h-2.5 rounded-full shrink-0 ${inc.resolved ? "bg-green-400" : "bg-yellow-400"}`}
                    />
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-cream/30 text-xs font-body">
                          {format(new Date(inc.incident_date + "T00:00:00"), "dd/MM/yyyy", { locale: ptBR })} às{" "}
                          {inc.incident_time}
                        </span>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button
                        onClick={() => resolveToggleMutation.mutate({ id: inc.id, resolved: inc.resolved })}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-gold/10">
                ...
                <button onClick={() => setIncidentModal(false)} className="text-cream/40 hover:text-cream">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <form
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setIncidentForm({ ...incidentForm, resolved: !incidentForm.resolved })}
          ...
          >
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => deleteIncidentMutation.mutate(deleteConfirm)}
