...
const AdminPix = () => {
  ...
  const confirmMutation = useMutation({
    mutationFn: async ({ reservaId, phone }: { reservaId: string; phone?: string }) => {
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  {isPending && (
                    <div className="flex flex-col gap-2 flex-shrink-0">
                      <button
                        onClick={() => confirmMutation.mutate({ reservaId: r.id, phone: guestPhone })}
                        disabled={confirmMutation.isPending}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 text-xs font-semibold font-body hover:bg-emerald-500/25 transition-all disabled:opacity-50"
                      >
