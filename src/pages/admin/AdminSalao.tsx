...
const AdminSalao = () => {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("hall_bookings").select("*").order("event_date", { ascending: true });
  ...
  return (
    <div className="min-h-screen bg-charcoal text-cream font-body">
      ...
      <header className="sticky top-0 z-40 bg-charcoal/95 backdrop-blur border-b border-gold/10 px-6 py-4 flex items-center justify-between">
        ...
        <Link to="/" className="text-xs text-cream/40 hover:text-primary transition">
          Ver Site →
        </Link>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <Link
              to="/admin"
              className="text-xs text-cream/40 hover:text-primary transition inline-flex items-center gap-1 mb-2"
            >
          ...
          <button
            onClick={openCreate}
            className="flex items-center gap-2 bg-gradient-to-r from-[#C9A84C] to-[#E5C97A] text-black font-semibold text-sm rounded-xl px-5 py-2.5 hover:scale-[1.02] transition-all shadow-lg"
          >
        ...
        <div className="bg-charcoal-light border border-gold/10 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="p-1.5 text-cream/40 hover:text-cream border border-gold/10 rounded-lg transition"
            >
        ...
        {/* Lista */}
        {isLoading ? (
          <p className="text-center text-cream/40 py-16 flex items-center justify-center gap-2">
            <Loader2 className="w-5 h-5 animate-spin" /> Carregando agendamentos...
          </p>
        ) : filtered.length === 0 ? (
                ...
                >
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${payCfg.color}`}>
                        {payCfg.label}
                      </span>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-cream/50">
                      <span className="flex items-center gap-1">
                        ...
                        {format(new Date(b.event_date + "T00:00:00"), "dd/MM/yyyy", { locale: ptBR })}
                      </span>
                      <span className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => openEdit(b)}
                        className="p-2 text-cream/40 hover:text-primary border border-gold/15 hover:border-primary/40 rounded-lg transition-all"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => setDeleteConfirm(b.id)}
                        className="p-2 text-cream/40 hover:text-red-400 border border-gold/15 hover:border-red-400/40 rounded-lg transition-all"
                      >
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input
                      className="w-full bg-charcoal-light border border-gold/10 rounded-lg px-3 py-2 text-sm text-cream placeholder:text-cream/30 focus:border-primary/40 focus:outline-none"
                  ...
                  <div>
                    ...
                    <select
                      className="w-full bg-charcoal-light border border-gold/10 rounded-lg px-3 py-2 text-sm text-cream focus:border-primary/40 focus:outline-none"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      className="w-full bg-charcoal-light border border-gold/10 rounded-lg px-3 py-2 text-sm text-cream focus:border-primary/40 focus:outline-none"
                      value={form.guests_count === 0 ? "" : form.guests_count}
                      placeholder="0"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      className="w-full bg-charcoal-light border border-gold/10 rounded-lg px-3 py-2 text-sm text-cream focus:border-primary/40 focus:outline-none"
                      value={form.total_price === 0 ? "" : form.total_price}
                      placeholder="0"
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea
                      className="w-full bg-charcoal-light border border-gold/10 rounded-lg px-3 py-2 text-sm text-cream placeholder:text-cream/30 focus:border-primary/40 focus:outline-none resize-none"
                      rows={3}
                      value={form.notes ?? ""}
                      onChange={(e) => setForm({ ...form, notes: e.target.value })}
                      placeholder="Pedidos especiais, decoração, cardápio..."
                    />
                  </div>
                </div>
