  ...
  return (
    <div className="relative flex-shrink-0">
      <button
        ...
        className="flex items-center gap-1 px-2 py-1.5 rounded border border-gold/30 hover:border-gold/60 text-cream/80 hover:text-primary transition-all duration-200 text-xs font-body tracking-wide"
      >
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-charcoal/95 backdrop-blur-md border-b border-gold/10">
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-16 md:h-20">
        {/* Logo — flex-shrink-0 garante que nunca encolhe */}
        <Link to="/" className="flex items-center gap-2 flex-shrink-0 min-w-0">
          <img
            src={hotelLogo}
            alt="Hotel SB - Sleep Better"
            className="h-9 md:h-12 w-auto object-contain flex-shrink-0"
            style={{ maxWidth: "44px" }}
          />
          <span className="font-body text-xs text-cream/40 tracking-widest uppercase hidden sm:block whitespace-nowrap">
            Sleep Better
          </span>
        </Link>
        ...
        {/* Mobile — controles direita */}
        <div className="lg:hidden flex items-center gap-2 flex-shrink-0">
          <LanguageSelector lang={lang} setLang={setLang} />
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="p-1 text-cream/80 hover:text-primary transition-colors"
            aria-label="Menu"
          >
      ...
      {/* Mobile Menu — animação rápida e sem lag */}
